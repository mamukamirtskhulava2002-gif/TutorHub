...
function previewContent(content) {
  ...
  if (content.startsWith("[FORWARD:")) return "↪️ გადაგზავნილი";
...
function ParentMessagesContent() {
  ...
  async function loadChats(user, supabase, userParam) {
    ...
    // Always include children in chat list even without messages
  ...
  function handleTypingBroadcast(value) {
    setMessage(value);
  ...
  async function deleteMessage(msgId) {
    ...
    const supabase = createClient();
    await supabase.from("messages").update({ content: "[DELETED]" })
  ...
  async function handleFileUpload(e, type = "file") {
    ...
    if (uploadError) { alert("ატვირთვა ვერ მოხერხდა."); setUploading(false); return; }
    const { data: { publicUrl } } = supabase.storage.from("chat-files").getPublicUrl(path);
    const isImage = /^image\//i.test(file.type);
    const isVideo = /^video\//i.test(file.type) || type === "video";
    const content = isImage ? `[IMAGE]${publicUrl}[/IMAGE]`
      : isVideo ? `[VIDEO]${publicUrl}[/VIDEO]`
      : `[FILE]${file.name}|${publicUrl}[/FILE]`;
    await supabase.from("messages").insert({
      sender_id: currentUser.id, receiver_id: activeChatId, content, seen: false,
    });
    setUploading(false);
  ...
  function renderContent(content, mine) {
    ...
    if (forwardMatch) {
      ...
      return (
        <div>
          <p className={`text-[11px] font-semibold mb-1.5 ${mine ? "text-emerald-200" : "text-gray-500"}`}>
    ...
    if (videoMatch) {
      return <video src={videoMatch[1]} controls preload="metadata"
    ...
    const oldMatch = content.match(/📎 \[ფაილი\]\((.*?)\)/);
    if (oldMatch) {
      const url = oldMatch[1];
  ...
  function shouldShowDate(idx) {
    ...
    return new Date(messages[idx - 1].created_at).toDateString() !==
           new Date(messages[idx].created_at).toDateString();
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 grid md:grid-cols-[240px_1fr]">
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4"
          ...
          <div className="bg-white rounded-2xl p-5 w-80 max-h-[70vh] flex flex-col shadow-2xl"
            ...
            <div className="flex-1 overflow-y-auto space-y-0.5 min-h-0">
              {chats.map(chat => (
                <div key={chat.id} onClick={() => setForwardTargetId(chat.id)}
                  className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all ${
                    forwardTargetId === chat.id ? "bg-emerald-50 border border-emerald-200" : "hover:bg-gray-50"
                  }`}>
                  <div className="avatar w-9 h-9 avatar-green text-sm shrink-0">{chat.name?.[0] || "?"}</div>
                  <p className="text-sm font-medium text-gray-800">{chat.name}</p>
                </div>
              ))}
            </div>
            <div className="flex gap-2 mt-4 pt-3 border-t border-gray-100">
              <button onClick={() => { setForwardModalOpen(false); setForwardMsg(null); setForwardTargetId(""); }}
                className="btn-secondary flex-1 py-2 text-sm">გაუქმება</button>
              <button onClick={() => handleForward(forwardTargetId)} disabled={!forwardTargetId}
                className="btn-primary flex-1 py-2 text-sm disabled:opacity-40 disabled:cursor-not-allowed">გაგზავნა</button>
            </div>
          </div>
        </div>
      )}
      ...
      {previewFile && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4"
          ...
          <div className="relative max-w-3xl max-h-[90vh]" onClick={e => e.stopPropagation()}>
            <img src={previewFile.url} alt="Preview" className="max-w-full max-h-[85vh] rounded-2xl object-contain" />
            <div className="flex gap-3 mt-4 justify-center">
              <a href={previewFile.url} download target="_blank" rel="noopener noreferrer"
                className="btn-primary px-6 py-2.5 text-sm" onClick={e => e.stopPropagation()}>⬇️ გადმოწერა</a>
              <button onClick={() => setPreviewFile(null)} className="btn-secondary px-6 py-2.5 text-sm">დახურვა</button>
            </div>
          </div>
        </div>
      )}
      ...
      <main className="flex flex-col" style={{ height: "100vh", overflow: "hidden" }}>
        <div className="flex flex-1 min-h-0 p-6 pb-0 md:p-8 md:pb-0">
          <div className="flex w-full bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-6"
                ...
                {loading ? (
                  ...
                  <div className="p-2 space-y-0.5">
                    {chats.map(chat => {
                      ...
                      return (
                        <div key={chat.id}
                          onClick={() => { setActiveChatId(chat.id); setReplyTo(null); setUnreadCounts(prev => ({ ...prev, [chat.id]: 0 })); }}
                          className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all ${
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between gap-1">
                              ...
                              {last?.created_at && (
                                <p className="text-[11px] text-gray-400 shrink-0">{relativeTimeShort(last.created_at)}</p>
                              )}
                            </div>
                            <div className="flex items-center justify-between gap-1">
                              <p className={`text-xs truncate ${unread > 0 ? "text-gray-700 font-medium" : "text-gray-400"}`}>
                                {last ? previewContent(last.content) : "💬 საუბრის დაწყება"}
                              </p>
                              {unread > 0 && (
            ...
            {activeChatId && activeChat ? (
              <div className="flex-1 flex flex-col min-w-0">
                <div className="px-4 py-3 border-b border-gray-100 flex items-center gap-3 shrink-0">
                  <div className="avatar w-9 h-9 avatar-green text-sm shrink-0">{activeChat.name?.[0]}</div>
                  <div className="flex-1 min-w-0">
                    ...
                    {messages.length === 0 ? (
                      ...
                      <div className="flex flex-col">
                        {messages.map((msg, idx) => {
                          ...
                          const isMedia    = msg.content.includes("[IMAGE]") || msg.content.includes("[FILE]") || msg.content.includes("[VIDEO]");
                          ...
                          if (!isMedia && !isDeleted) {
                            bubbleRound = mine
                              ? `rounded-2xl ${first ? "rounded-tr-md" : ""} ${last ? "rounded-br-md" : "rounded-r-md"}`
                              : `rounded-2xl ${first ? "rounded-tl-md" : ""} ${last ? "rounded-bl-md" : "rounded-l-md"}`;
                          }
                            ...
                            <div key={msg.id}>
                              {showDate && (
                                <div className="flex items-center gap-3 my-4">
                                  ...
                                  <span className="text-xs text-gray-400 shrink-0">{dateSeparatorLabel(msg.created_at)}</span>
                                  <div className="flex-1 h-px bg-gray-100" />
                                </div>
                              )}
                              ...
                              <div
                                ...
                                {!mine && (
                                  <div className={`w-7 h-7 rounded-full shrink-0 text-xs font-bold flex items-center justify-center ${last ? "bg-emerald-100 text-emerald-800" : "opacity-0"}`}>
                                ...
                                {mine && !isDeleted && isHovered && (
                                  <div className="flex items-center gap-0.5 shrink-0 mb-5">
                                    ...
                                    <button onClick={() => startForward(msg)} title="↪ Forward"
                                      className="w-6 h-6 bg-white border border-gray-200 rounded-full shadow-sm flex items-center justify-center text-[11px] hover:border-blue-300 hover:bg-blue-50 transition-all shrink-0">↪</button>
                                    <button onClick={() => deleteMessage(msg.id)} disabled={deletingId === msg.id}
                                      title="წაშლა" className="w-6 h-6 bg-white border border-gray-200 rounded-full shadow-sm flex items-center justify-center text-xs hover:bg-red-50 hover:border-red-200 transition-all disabled:opacity-50 shrink-0">
                                      {deletingId === msg.id ? "⋯" : "🗑"}
                                    </button>
                                  </div>
                                )}
                                ...
                                <div className="flex flex-col max-w-xs">
                                  <div className={`${isMedia || isDeleted ? "" : mine
                                    ...
                                    {!isMedia && (
                                      <p className={`text-[11px] mt-0.5 ${isDeleted ? "text-gray-400" : mine ? "text-emerald-200" : "text-gray-400"}`}>
                                        {formatTime(msg.created_at)}
                                      </p>
                                    )}
                                    {isMedia && <p className="text-[11px] text-gray-400 mt-0.5">{formatTime(msg.created_at)}</p>}
                                  </div>
                ...
                <div className="p-4 border-t border-gray-100 shrink-0">
                  {replyTo && (
                    <div className="flex items-center gap-2 mb-2 px-2 py-1.5 bg-emerald-50 rounded-xl border border-emerald-200">
                      ...
                      <button onClick={() => setReplyTo(null)} className="text-gray-400 hover:text-gray-600 text-sm px-1 shrink-0">✕</button>
                    </div>
                  )}
                  {uploading && (
                  ...
                  <div className="flex gap-2 items-end">
                        ...
                        <div className="absolute bottom-12 left-0 bg-white border border-gray-200 rounded-2xl shadow-xl p-3 grid grid-cols-8 gap-1 z-20 w-72">
                          {EMOJIS.map(emoji => (
                            <button key={emoji} type="button"
                              onClick={() => { setMessage(prev => prev + emoji); setEmojiOpen(false); textareaRef.current?.focus(); }}
                    ...
                    <label className="shrink-0 w-10 h-10 flex items-center justify-center rounded-xl border border-gray-200 text-gray-400 hover:text-emerald-600 hover:border-emerald-300 cursor-pointer transition-all">
                      🖼️
                    ...
                    <label className="shrink-0 w-10 h-10 flex items-center justify-center rounded-xl border border-gray-200 text-gray-400 hover:text-emerald-600 hover:border-emerald-300 cursor-pointer transition-all">
                      🎥
                      <input ref={videoInputRef} type="file" className="hidden" accept="video/mp4,video/webm,video/ogg,video/quicktime"
                        onChange={e => handleFileUpload(e, "video")} disabled={uploading} />
                    </label>
                    <label className="shrink-0 w-10 h-10 flex items-center justify-center rounded-xl border border-gray-200 text-gray-400 hover:text-emerald-600 hover:border-emerald-300 cursor-pointer transition-all">
                      📎
                      <input ref={fileInputRef} type="file" className="hidden" accept=".pdf,.doc,.docx,.xls,.xlsx,.txt,.zip,.rar"
                        onChange={e => handleFileUpload(e, "file")} disabled={uploading} />
                    </label>
                    <textarea ref={textareaRef} rows={1} placeholder="ჩაწერეთ შეტყობინება..."
                      value={message}
                      onChange={e => { handleTypingBroadcast(e.target.value); autoResize(e.target); }}
                      onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleSend(); } }}
                      className="input flex-1 resize-none overflow-hidden leading-relaxed py-2.5"
                      style={{ minHeight: "42px", maxHeight: "120px" }} disabled={uploading} />
                    <button onClick={handleSend} disabled={!message.trim() || uploading}
