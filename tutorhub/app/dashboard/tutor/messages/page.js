...
function TutorMessagesContent() {
  ...
  useEffect(() => {
    async function init() {
      ...
      await loadChats(user, supabase, studentParam);
      setLoading(false);
    }
    init();
  }, [searchParams]);
  ...
  // ─── chat list ───────────────────────────────────────────────────────────
  const loadChats = useCallback(async (user, supabase, studentParam) => {
    ...
    if (studentParam && !seen.has(studentParam)) otherIds.unshift(studentParam);
  ...
  useEffect(() => {
    ...
    const supabase    = createClient();
    const studentParam = searchParams.get("student");
    const interval = setInterval(() => loadChats(currentUser, supabase, studentParam), 5000);
    return () => clearInterval(interval);
  }, [currentUser, loadChats, searchParams]);
  ...
  async function handleFileUpload(e, type = "file") {
    ...
    if (uploadError) {
      alert(`ატვირთვა ვერ მოხერხდა: ${uploadError.message}`);
      setUploading(false);
      return;
    }
              ...
              <div className="flex-1 overflow-y-auto">
                {loading ? (
                  ...
                  <div className="text-center py-12 px-4">
                    ...
                    <p className="text-xs text-gray-300 mt-1">სტუდენტები თქვენ მოგწერენ</p>
                  </div>
                ) : (
                      ...
                      <div className="flex flex-col">
                          ...
                          return (
                            <div key={msg.id}>
                              ...
                              >
                                {/* Partner avatar */}
                                {!mine && (
                                  <div className={`w-7 h-7 rounded-full shrink-0 text-xs font-bold flex items-center justify-center bg-blue-100 text-blue-700 ${
                                    last ? "" : "opacity-0"
                                  }`}>
                                ...
                                {/* Action buttons — mine side */}
                                {mine && !isDeleted && hoveredMsgId === msg.id && (
                        ...
                        {isPartnerTyping && (
                          <div className="flex justify-start mt-3">
                            <div className="w-7 h-7 rounded-full mr-2 self-end bg-blue-100 text-blue-700 text-xs font-bold flex items-center justify-center shrink-0">
                              {activeChat.name?.[0]}
                            </div>
